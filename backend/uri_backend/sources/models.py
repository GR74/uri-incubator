...
class Artifact(Base):
    ...
    storage_key: Mapped[str] = mapped_column(sa.String(128), nullable=False, unique=True)
...
class Source(Base):
    ...
    __table_args__ = (sa.UniqueConstraint("project_id", "family", "external_id", name="uq_source_project_identity"),)
...
class SourceVersion(Base):
    ...
    __table_args__ = (sa.UniqueConstraint("project_id", "family", "external_id", "native_version", "artifact_id", name="uq_source_version_idempotency"),)

    id: Mapped[UUID] = mapped_column(sa.Uuid, primary_key=True, default=uuid4)
    source_id: Mapped[UUID] = mapped_column(sa.ForeignKey("sources.id"), nullable=False)
    project_id: Mapped[UUID] = mapped_column(sa.ForeignKey("projects.id"), nullable=False)
    artifact_id: Mapped[UUID] = mapped_column(sa.ForeignKey("artifacts.id"), nullable=False)
    family: Mapped[str] = mapped_column(sa.String(64), nullable=False)
    external_id: Mapped[str] = mapped_column(sa.String(500), nullable=False)
...
class ContentPart(Base):
    ...
    __table_args__ = (sa.UniqueConstraint("source_version_id", "ordinal", name="uq_content_part_ordinal"),)
