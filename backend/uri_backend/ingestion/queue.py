...
async def enqueue_ingestion(session: AsyncSession, source_version_id: UUID, idempotency_key: str) -> IngestionRun:
    run_id = await session.scalar(pg_insert(IngestionRun).values(source_version_id=source_version_id, pipeline_version=idempotency_key, idempotency_key=idempotency_key, status="queued").on_conflict_do_nothing(constraint="uq_ingestion_run_source_pipeline").returning(IngestionRun.id))
    if run_id is None:
        run_id = await session.scalar(sa.select(IngestionRun.id).where(IngestionRun.source_version_id == source_version_id, IngestionRun.pipeline_version == idempotency_key))
    assert run_id is not None
    await session.execute(pg_insert(IngestionJob).values(run_id=run_id, status="queued").on_conflict_do_nothing(index_elements=[IngestionJob.run_id]))
    run = await session.get(IngestionRun, run_id)
    assert run is not None
    return run


async def claim_next_job(session: AsyncSession, worker_id: str, lease_seconds: int) -> ClaimedJob | None:
    if lease_seconds <= 0:
...
async def _owned_job(session: AsyncSession, job_id: UUID, worker_id: str) -> IngestionJob:
    job = await session.scalar(sa.select(IngestionJob).where(IngestionJob.id == job_id, IngestionJob.status == "running", IngestionJob.worker_id == worker_id, IngestionJob.lease_expires_at > sa.func.now()).with_for_update())
    if job is None:
        raise JobLeaseLost("job lease is no longer owned by this worker")
    return job


async def _finish_attempt(session: AsyncSession, job: IngestionJob, outcome: str, code: str | None = None, detail: str | None = None) -> None:
    attempt = await session.scalar(sa.select(IngestionJobAttempt).where(IngestionJobAttempt.job_id == job.id, IngestionJobAttempt.attempt == job.attempt).with_for_update())
...
async def heartbeat_job(session: AsyncSession, job_id: UUID, worker_id: str, lease_seconds: int) -> IngestionJob:
    if lease_seconds <= 0:
        raise ValueError("lease_seconds must be positive")
...
async def complete_job(session: AsyncSession, job_id: UUID, worker_id: str) -> IngestionJob:
    job, now = await _owned_job(session, job_id, worker_id), sa.func.now()
...
async def cancel_job(session: AsyncSession, job_id: UUID, worker_id: str) -> IngestionJob:
    job, now = await _owned_job(session, job_id, worker_id), sa.func.now()
